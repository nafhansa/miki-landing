...
export function HeroScene({ textRefs, cameraPositions = DEFAULT_CAMERA_POSITIONS, cameraFov = 50, model }: HeroSceneProps) {
  ...
  return (
    <>
      ...
      <spotLight position={[-5, 5, -5]} intensity={5} color="#00d2ff" angle={0.5} penumbra={1} />
      <spotLight position={[5, 0, 5]} intensity={2} color="#bd00ff" angle={1} penumbra={1} />
      <Environment preset="city" />

      <PerspectiveCamera
        makeDefault
        position={[cameraPositions.start.x, cameraPositions.start.y, cameraPositions.start.z]}
        fov={cameraFov}
      />
